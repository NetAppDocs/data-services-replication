...
Delete the data protection relationship between the source and destination volumes, deleting the SnapMirror relationship from the destination site and releases the relationship from the source site. Deleting the relationship also breaks the SnapMirror relationship, which briefly makes the destination volume the primary site for read/write operations to ensure the destination can be used independently after replication is removed.
