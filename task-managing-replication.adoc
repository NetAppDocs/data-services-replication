...
| Delete 
a| Delete the data protection relationship between the source and destination volumes. This ends data replication between volumes.

Deleting does not activate the destination volume for data access.

[NOTE]
Deleting the relationship also breaks the SnapMirror relationship, which briefly makes the destination volume the primary site for read/write operations. 
|===
