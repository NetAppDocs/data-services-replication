...
Replication relationships with FSx for ONTAP file systems as source or target are managed in NetApp Workload Factory. You can manage these relationships from the Console systems page by selecting the FSx for ONTAP file system. 

.Steps

. link:task-login-replication.html[Log in to NetApp Replication]. 
+
Continue to step 2 to manage non-FSx for ONTAP replication relationships. 
+
To manage replication relationships with FSx for ONTAP file systems, after you log in to the NetApp Console, go to *Storage* > *Management* > *Systems*. Then select the FSx for ONTAP file system. Now you're in Workload Factory in the FSx for ONTAP resource screen. Select the *Replication relationships* tab to view and manage the replication relationships for the file system.
...
| Delete 
a| Delete the data protection relationship between the source and destination volumes. This ends data replication between volumes, deletes the SnapMirror relationship from the destination site, and releases the relationship from the source site. 

Deleting does not activate the destination volume for data access.

[NOTE]
Deleting the relationship also breaks the SnapMirror relationship, which briefly makes the destination volume the primary site for read/write operations to ensure the destination can be used independently after replication is removed.
|===
